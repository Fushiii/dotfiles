...
func keyBindings(mwg *sync.WaitGroup) {
	...
	launch_path := variables.RiverConfigVar.Path + "/configs/rofi/launch.sh "
	launch_selector_path := launch_path + "selector "

	launcher := launch_path + "launcher"
	powermenu := launch_path + "powermenu"

	wallpaper_selector := launch_selector_path + "wallpaper"

	// List of Keybinings
	allCMDs := []*exec.Cmd{
		// river
		exec.Command(RIVERCTL, MAP, NORMAL, "Super+Shift", "R", "riverctl", "spawn", variables.RiverConfigVar.Path+"/init"),
		...
		exec.Command(RIVERCTL, MAP, NORMAL, "Super", "X", SPAWN, powermenu),
		// Selectors
		exec.Command(RIVERCTL, MAP, NORMAL, "Super", "T", SPAWN, wallpaper_selector),
		...
		// exec.Command(RIVERCTL, MAP, NORMAL, "Super", "L", "focus-view", "left"),
		// exec.Command(RIVERCTL, MAP, NORMAL, "Super", "H", "focus-view", "right"),
