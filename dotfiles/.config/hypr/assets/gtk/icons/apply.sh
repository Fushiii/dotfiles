...
csv_file="$HOME/.config/hypr/configs/taste/icons.csv"
hypr_icons_color_path="$HOME/.config/hypr/assets/gtk/icons/color"
hypr_icons_theme_path="$HOME/.config/hypr/assets/gtk/icons/theme"
...
check_icons() {
	...
	if [ ! -d "$cached_icons_path" ]; then
		...
		*)
		...
		echo "$color" >"$hypr_icons_color_path"
		echo "$theme" >"$hypr_icons_theme_path"
	fi
}
...
recolor_icons() {
	...
	if [ -f "$hypr_icons_color_path" ]; then
		hypr_icons_color=$(cat "$hypr_icons_color_path")
		sanitized_hypr_icons_color=$(echo "${color#"#"}" | tr -d '\r\n[:space:]')
		if [ "$hypr_icons_color" != "$color" ]; then
			echo "Replacing the icons as colors do not coincide: $hypr_icons_color $color"
			check_icons $theme
		fi
	else
		check_icons $theme
...
main() {
	...
	while [[ $# -gt 0 ]]; do
		...
		check)
			shift
			previous_theme=$(cat "$hypr_icons_theme_path")
