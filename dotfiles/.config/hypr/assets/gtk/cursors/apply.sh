...
csv_file="$HOME/.config/hypr/configs/taste/cursors.csv"
hypr_cursors_color_path="$HOME/.config/hypr/assets/gtk/cursors/color"
hypr_cursors_theme_path="$HOME/.config/hypr/assets/gtk/cursors/theme"
cursors_assets_path="$HOME/.icons/assets/cursors"
...
check_cursors() {
	...
	if [ ! -d "$cached_cursors_path" ]; then
		...
		*)
		...
		echo "$color" >"$hypr_cursors_color_path"
		echo "$theme" >"$hypr_cursors_theme_path"
	fi
}
...
recolor_cursors() {
	...
	if [ -f "$hypr_cursors_color_path" ]; then
		hypr_cursors_color=$(cat "$hypr_cursors_color_path")
		sanitized_hypr_cursors_color=$(echo "${color#"#"}" | tr -d '\r\n[:space:]')
		if [ "$hypr_cursors_color" != "$color" ]; then
			echo "Replacing the cursors as colors do not coincide: $hypr_cursors_color $color"
			check_cursors $theme
		fi
	else
		check_cursors $theme
...
main() {
	...
	while [[ $# -gt 0 ]]; do
		...
		check)
			shift
			previous_theme=$(cat "$hypr_cursors_theme_path")
