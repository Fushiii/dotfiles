...
hypr_wallpaper_theme="${hypr_wallpaper_dir}/theme"
hypr_wallpaper_name="${hypr_wallpaper_dir}/name"
...
select_wallpapers() {
    find "${wallpaper_collection_dir}/dark" -type f \( -iname "*.jpg" -o -iname "*.jpeg" -o -iname "*.png" -o -iname "*.webp" \) -print0
}
...
main() {
    if [ -z "${1-}" ]; then
        ...
        echo "dark" >"${hypr_wallpaper_theme}"
        RECOLOR=true
        coproc (flavours -c "$hypr_config_flavours" apply --theme dark)
    fi
}
