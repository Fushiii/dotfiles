...
river_wallpaper_color_path="$HOME/.config/river/assets/wallpaper/color"
river_wallpaper_recolor_path="$HOME/.config/river/assets/wallpaper/status"
river_wallpaper_theme_path="$HOME/.config/river/assets/wallpaper/theme"
river_wallpaper_name_path="$HOME/.config/river/assets/wallpaper/name"
wallpaper_assets_path="$HOME/.icons/assets/wallpaper"
wallpaper_path="$HOME/.config/river/assets/wallpaper/wallpaper"
...
check_wallpaper() {
	...
	if [ ! -f "$cached_wallpaper" ]; then
		...
		echo "$color" >"$river_wallpaper_color_path"
		echo "$theme" >"$river_wallpaper_theme_path"
		echo "$image" >"$river_wallpaper_name_path"
	fi
...
recolor_wallpaper() {
	...
	if [ -f "$river_wallpaper_recolor_path" ]; then
		rm "$river_wallpaper_recolor_path"
		check_wallpaper $theme $image
	else
		if [ -f "$river_wallpaper_color_path" ]; then
			river_wallpaper_color=$(cat "$river_wallpaper_color_path")

			if [ "$river_wallpaper_color" != "$color" ]; then
				echo "Replacing the wallpaper as colors do no coincide: $river_wallpaper_color $color"
				check_wallpaper $theme $image
			fi
...
main() {
	local theme="dark"
	image=$(cat "$river_wallpaper_name_path")
	...
	while [[ $# -gt 0 ]]; do
		...
		check)
			shift
			previous_theme=$(cat "$river_wallpaper_theme_path")
			pkill swww
			swww init
			swww clear
	...
	if [ "$RECOLOR_ALL" = "true" ] || [ "$RECOLOR_WALLPAPER" = "true" ]; then
		case "$theme" in
		dark)
